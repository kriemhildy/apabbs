psql $DATABASE_URL \
    --command="SELECT pub_id, media_file_name FROM posts WHERE thumbnail_file_name IS NULL \
        AND status = 'approved' \
...
| while read -r pub_id media_file_name; do
    echo "Generating thumbnail for pub/media/$pub_id/$media_file_name"
    vipsthumbnail \
        --size "1400x1600>" \
        --eprofile=srgb \
        --output "tn_%s.jpg[optimize_coding,strip]" \
        "pub/media/$pub_id/$media_file_name"
