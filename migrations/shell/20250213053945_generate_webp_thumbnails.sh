psql $DATABASE_URL \
    --command="SELECT uuid, media_file_name FROM posts \
...
| while read -r uuid media_file_name; do
    echo "Generating webp thumbnail for pub/media/$uuid/$media_file_name"
    # extension=$(echo "${media_file_name##*.}" | tr '[:upper:]' '[:lower:]')
    extension=$(echo "${media_file_name##*.}" | tr '[:upper:]' '[:lower:]')
    echo "Extension: $extension"
    vips_input_file_path="pub/media/$uuid/$media_file_name"
