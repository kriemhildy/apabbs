psql $DATABASE_URL \
    --command="SELECT uri, media_file_name FROM posts \
        WHERE status = 'approved' AND media_category = 'image';" \
    --quiet \
    --no-align \
    --tuples-only \
    --field-separator ' ' \
| while read -r uri media_file_name; do
    echo "Generating webp thumbnail for pub/media/$uri/$media_file_name"
    # extension=$(echo "${media_file_name##*.}" | tr '[:upper:]' '[:lower:]')
    extension=$(echo "${media_file_name##*.}" | tr '[:upper:]' '[:lower:]')
    echo "Extension: $extension"
    vips_input_file_path="pub/media/$uri/$media_file_name"
    ...
    psql $DATABASE_URL \
       --command="UPDATE posts SET thumbnail_file_name = '$thumbnail_file_name' WHERE uri = '$uri';"
    old_jpg_thumbnail="pub/media/$uri/tn_$file_name_without_extension.jpg"
